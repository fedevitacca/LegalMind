...
const { claimNextJob, completeJob, completeIndexJob, completeOcrJob, failJob, updateJobProgress } = require("../modelos/repositorioTrabajosDocumentales");
...
async function processClaimedJob(job,deps={readFile:fs.readFile,extract:extractTextFromSupportedFile,complete:completeJob,completeIndex:completeIndexJob,completeOcr:completeOcrJob,index:indexDocument,recognize:recognizeDocument,progress:updateJobProgress,fail:failJob}){
  try {if(job.tipo==="indexar_rag"){await deps.index(job);await deps.completeIndex(job);return;}if(job.tipo==="ocr"){if(!job.ruta_archivo)throw new Error("OCR_FILE_MISSING: no existe original para procesar.");const result=await deps.recognize(job.ruta_archivo,{onProgress:(value)=>void deps.progress(job.id,10+value*.85)});await deps.completeOcr(job,result);return;}
