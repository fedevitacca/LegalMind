...
type AnalysisMetadata = { duration_ms?: number; model?: string; semantic_retrieval?: string; evidence?: Evidence; grounding?: Grounding };
...
export default function CentroAnalisisIA() {
  ...
  async function execute() {
    setBusy(true); setError(""); setResult(undefined); setCitations([]); setSavedId(undefined); setMetadata(undefined); setEvidence(undefined); setGrounding(undefined);
    ...
    try {
      ...
      setResult(body.result); setCitations(body.citations || []); setSavedId(body.saved_query?.id); setMetadata(body._metadata); setEvidence(body.evidence); setGrounding(body.grounding);
