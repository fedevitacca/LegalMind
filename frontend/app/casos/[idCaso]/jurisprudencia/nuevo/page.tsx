...
import FormularioNuevaJurisprudencia from "../../../../../components/casos/FormularioNuevaJurisprudencia";
import MarcoAplicacion from "../../../../../components/estructura/MarcoAplicacion";
import BotonSesion from "../../../../../components/interfaz/BotonSesion";

export default async function PaginaNuevaJurisprudencia({
  params,
}: {
    ...
    <MarcoAplicacion activeSection="Casos">
      <section className="h-full min-h-0 overflow-y-auto bg-[#F4F7F5] text-[#0F2044]">
        ...
        <main className="px-9 py-9">
          <section className="mb-[50px] flex h-[90px] w-[340px] items-center gap-3 rounded-[23px] border-2 border-[#88A9C8] bg-white px-5 text-[28px] leading-none">
            <BookIcon className="h-9 w-9" />
            Cargar jurisprudencia
          </section>

          <FormularioNuevaJurisprudencia caseId={idCaso} />
        </main>
      </section>
    </MarcoAplicacion>
  );
...
function BookIcon({ className = "h-8 w-8" }: { className?: string }) {
  return (
    <svg aria-hidden="true" className={className} viewBox="0 0 24 24" fill="none">
      <path d="M4 5.5c2.7-1 5.2-.8 8 1v13c-2.8-1.8-5.3-2-8-1v-13ZM12 6.5c2.8-1.8 5.3-2 8-1v13c-2.7-1-5.2-.8-8 1v-13Z" stroke="currentColor" strokeLinejoin="round" strokeWidth="2" />
      <path d="M12 6.5v13" stroke="currentColor" strokeLinecap="round" strokeWidth="2" />
    </svg>
  );
}
